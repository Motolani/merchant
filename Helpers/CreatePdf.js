...
import ShagoLogoBase64 from "./ShagoLogoBase64__";
    ...
    if (await isPermitted()) {
      ...
      const productLogoTag = image
        ? await convertImageToBase64(image).then(
            (res) =>
              `<img src="data:image/png;base64,${res}" alt="Shago" style="${imageStyle}" ></img>`
          )
        : "";

      const contactInfo = `<div style="${contactInfoStyle}"> <div>CUSTOMER CARE</div> <div><span>+2349090000448,</span>  <span>+2349090000844</span></div>  <div>customercare@shagopayments.com</div> </div>`;
      ...
      if (
        !printObjectsArray.find(
          (el) => el.label.toString().toLocaleLowerCase() == "date"
        )
      ) {
        htmlBody += `<div style="${row}"><div style="${col_sm}">Date</div><div style="${col_sm_right}">${dateFormat(
          new Date(),
          "dd/mm/yyyy hh:MM:ss tt"
        )}</div></div><hr>`;
      }

      const html = `<div style="${wrapper}"><div ><div style="${logo}">${productLogoTag}</div><div style="${main}">${htmlBody}</div><div style="${footer}"> <div>${shagoLogoTag}</div> <div style="${thankYouMessage}">Thank you for using Shago</div> ${contactInfo} </div></div></div>`;
